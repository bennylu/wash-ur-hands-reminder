...
addReminder = () => {
  ...
  if (hour == "" || minute == "" || text == "") return;

  addReminderAsync(hour, minute, text).then(() => {
    inflateReminders();
  });
};

deleteReminder = name => {
  deleteReminderAsync(name).then(() => {
    inflateReminders();
  });
};
...
inflateReminders = async () => {
  ...
  if (names.length > 0) {
    ...
    let header = $(
      '<tr><th width="20%">時間</th><th width="60%">提醒事項</th><th>刪除</th></tr>'
    );
    tb.append(header);
    ...
    names.forEach(name => {
      ...
      tr.append(
        '<td><a id="del-' +
          name +
          '"><div class="btn btn-danger">刪除</div></a></td>'
      );
      tb.append(tr);
